...
from tetherxyz_latest import create_viewer,G,CAD,OPTICS
SOURCE=OPTICS
v=create_viewer()
...
for renderer in (v.wide,v.detail,v.reconstruction,v.left,v.right):
    for actor in list(renderer.GetActors2D()):
        ...
        elif '30 mm camera baseline / Quarton' in label:actor.SetInput('30 mm camera baseline / centered line laser\nReinforced supports and revised swivel pulleys')
...
(ROOT/'scripts/laser-stereo-media.json').write_text(json.dumps({'source':str(SOURCE),'cad':str(CAD/'XYZ_V54_Serviceable_Scanner.FCStd'),'cad_sha256':hashlib.sha256((CAD/'XYZ_V54_Serviceable_Scanner.FCStd').read_bytes()).hexdigest(),'source_sha256':{n:hashlib.sha256((SOURCE/n).read_bytes()).hexdigest() for n in ('viewer.py','design.py','scan.py')},'frames':count,'fps':fps,'duration_s':count/fps,'playback_speed':G.PLAYBACK_SPEED},indent=2))
v.window.Finalize()
