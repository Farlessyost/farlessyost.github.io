...
<section class="contact-page shell">
  ...
  <div class="contact-links">{% if site.email and site.email != '' %}<a href="mailto:{{ site.email }}"><span><small>Email</small>{{ site.email }}</span><span aria-hidden="true">↗</span></a>{% endif %}<a href="{{ site.linkedin }}"><span>LinkedIn</span><span aria-hidden="true">↗</span></a><a href="{{ '/resume.pdf' | relative_url }}?v={{ site.resume_version }}"><span>Résumé (PDF)</span><span aria-hidden="true">↗</span></a></div></div>
</section>
