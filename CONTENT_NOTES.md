...
The public scanner name is TetherFrame, at `/projects/tetherframe/`; `/projects/xyz-robot/` and `/projects/skopeo/` redirect directly to it. Skopeo was replaced after the owner identified an existing tool with that name. Exact-name and robotics/scanner/software web searches for TetherFrame on September 15, 2026 found no obvious competing product; incidental uses included an image label and a physics reference-frame term. This is a portfolio project label, not a claim of trademark or commercial-name clearance. Original project directories, CAD sources, scripts, and media filenames retain XYZ for continuity.
